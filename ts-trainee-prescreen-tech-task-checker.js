const {
  ...
  ***********************************************/
  function calculateTeamFinanceReport(salaries, team) {
    ...
    for (const member of team) {
      const specializationSalary = salaries[member.specialization]
      if (!specializationSalary) {
        continue
      }

      const cleanSalaryPercent = 100 - Number.parseInt(specializationSalary.tax)
      const cleanSalary = specializationSalary.salary / (cleanSalaryPercent / 100)

      specializationReportName = `totalBudget${member.specialization}`
      if (!result[specializationReportName]) {
        result[specializationReportName] = 0
      }

      result.totalBudgetTeam += cleanSalary
      result[specializationReportName] += cleanSalary
    }
  ...
  ***********************************************/
  ...
  const correctCalculateTeamFinanceReport = (pricing, team) => {
    ...
    const res = fullFilds.reduce((total, item) => {
      ...
      if (!total[`totalBudget${item.typePosition}`]) total[`totalBudget${item.typePosition}`] = 0;
      total[`totalBudget${item.typePosition}`] = total[`totalBudget${item.typePosition}`] + resNumber;
      return total;
    }, {
      totalBudgetTeam: 0
    });
  ...
  ***********************************************/
  describe('Base tests', () => {
    ...
    it("Dima's tests", () => {
      ...
      expect(calculateTeamFinanceReport(pricing1, team1)).toEqual(correctCalculateTeamFinanceReport(pricing1, team1));
      expect(calculateTeamFinanceReport(pricing2, team2)).toEqual(correctCalculateTeamFinanceReport(pricing2, team2));
      expect(calculateTeamFinanceReport(pricing3, team3)).toEqual(correctCalculateTeamFinanceReport(pricing3, team3));
      expect(calculateTeamFinanceReport(pricing4, team4)?.totalTaxTesters).toEqual(correctCalculateTeamFinanceReport(pricing4, team4)?.totalTaxTesters);
    });
    ...
    it("task explanation case", () => {
      ...
      expect(calculateTeamFinanceReport(pricing, team))
        .toEqual({
